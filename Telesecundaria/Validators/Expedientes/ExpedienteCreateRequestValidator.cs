...
{
    public class ExpedienteCreateRequestValidator : AbstractValidator<ExpedienteCreateRequest>
    {
        public ExpedienteCreateRequestValidator()
        {
            RuleFor(x => x.Nombre)
            ...
            RuleFor(x => x.TipoTitular)
                .NotEmpty().WithMessage("El tipo de titular es obligatorio.")
                .Must(SerTipoTitularValido)
                    .WithMessage("El tipo de titular debe ser 'Alumno' o 'Empleado'.");

          //  RuleFor(x => x.ClaveEntrega)
            //    .Matches(@"^ENTR-\d{16}$").WithMessage("El formato de la clave de entrega no es válido.")
           //     .When(x => !string.IsNullOrWhiteSpace(x.ClaveEntrega));


        }

        private bool SerTipoTitularValido(string tipoTitular)
        {
            var tiposValidos = new[] { "Alumno", "Empleado" };
            return tiposValidos.Contains(tipoTitular);
        }
